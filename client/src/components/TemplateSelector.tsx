...
const TemplateSelector: React.FC<TemplateSelectorProps> = ({
  ...
  const loadUserAccess = async () => {
    ...
    try {
      ...
      if (data.user) console.log('User data loaded:', data.user);
    } catch (error: any) {
        ...
        <div className="mb-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 text-xs border border-gray-200 rounded-lg focus:border-[#2c2a63] focus:ring-1 focus:ring-[#2c2a63]/20 outline-none"
            />
            {searchQuery && (
            ...
            {filteredTemplates.map((template) => {
              ...
              return (
                <div
                  ...
                  style={isSelected ? { border: '2px solid #2c2a63', borderRadius: 8, padding: 2 } : {}}
                >
                  <div
                  ...
                  >
                    ...
                    {template.isPremium && (
                      <div className="absolute top-1.5 right-1.5 z-10">
                        <Crown className="w-4 h-4 text-yellow-500 drop-shadow-lg" fill="currentColor" />
                      </div>
                    )}
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
                    {isSelected && (
                      <div className="absolute top-1.5 left-1.5 z-10">
                        <div className="w-5 h-5 rounded-full flex items-center justify-center shadow-lg" style={{ backgroundColor: '#2c2a63' }}>
                          <Check className="w-3 h-3 text-[#EDC9AF]" strokeWidth={3} />
                        </div>
                      </div>
                    )}
                  </div>
                  <p className="text-[10px] font-medium text-gray-700 mt-1 px-0.5 truncate">{template.name}</p>
                </div>
              );
            })}
          </div>
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                style={isActive ? { backgroundColor: '#2c2a63', color: '#EDC9AF' } : { backgroundColor: '#f3f4f6', color: '#4b5563' }}
              >
                {cat}
              </button>
            );
            ...
            {filteredTemplates.map((template) => {
              ...
              return (
                <div
                  ...
                  style={isSelected ? { border: `2px solid #2c2a63`, padding: '2px' } : {}}
                >
                  <div
                    className="relative aspect-[3/4] rounded-lg overflow-hidden bg-gray-100 shadow-sm group-hover:shadow-md transition-shadow"
                  ...
                  <div className="mt-2 px-1">
                    <h3 className={`text-sm font-medium transition-colors ${isSelected ? "text-gray-900" : "text-gray-700 group-hover:text-gray-900"}`}>
                      {template.name}
                    </h3>
                  </div>
                </div>
