...
function calculateRuleBasedScore(resumeText, parsedData, jobDescription) {
    ...
    if (jobDescription) {
        ...
        if (matchRate >= 0.70)
            ...
            keywordsScore = 50;
        else if (matchRate >= 0.40)
            keywordsScore = 40;
        else if (matchRate >= 0.30)
            keywordsScore = 30;
        else if (matchRate >= 0.20)
            keywordsScore = 20;
        else
            keywordsScore = 10;
        ...
        if (keywordDensity >= 0.60)
            keywordsScore = 60;
        else if (keywordDensity >= 0.45)
            keywordsScore = 50;
        else if (keywordDensity >= 0.30)
            keywordsScore = 40;
        else if (keywordDensity >= 0.20)
            keywordsScore = 30;
        else
            keywordsScore = 20;
    ...
    const hasProjectMetrics = projects.some(p => p.description && (p.description.match(/\d+|users|performance|efficiency/i)));
    ...
    let overallScore = Math.round(keywordsScore * weights.keywordsMatch +
...
function calculateMissingSectionsPenalty(parsedData) {
...
export const analyzeResume = async (file, jobDescription) => {
    try {
        ...
        if (finalScore >= 70)
            ...
            interpretation = '📊 AVERAGE - Industry Standard';
        else if (finalScore >= 30)
            interpretation = '📉 BELOW AVERAGE - Needs Improvement';
        else
            interpretation = '❌ POOR - Requires Major Revision';
