...
export const updateEbook = async (req, res) => {
    try {
        ...
        const ebook = await Ebook.findByIdAndUpdate(ebookId, {
            $set: {
...
export const toggleEbookStatus = async (req, res) => {
    try {
        const { ebookId } = req.params;
        const { isActive } = req.body;
        const ebook = await Ebook.findByIdAndUpdate(ebookId, { $set: { isActive: Boolean(isActive) } }, { new: true });
        if (!ebook) {
            res.status(404).json({ message: "Ebook not found" });
            return;
        }
