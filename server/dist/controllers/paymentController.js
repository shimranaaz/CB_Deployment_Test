...
const normalizePlan = (planNameOrKey) => {
    const planMap = {
        "free": "Free",
        "trial": "Trial",
        "starter": "Basic",
...
const calculatePlanExpiration = (plan) => {
...
export const verifyPayment = async (req, res) => {
    try {
        ...
        if (!isValid) {
            await Payment.findOneAndUpdate({ razorpay_order_id }, { status: "failed", failureReason: "Invalid signature" });
...
export const checkTemplateAccess = async (req, res) => {
    try {
        const userId = req.userId;
...
// ===================== CHECK RESUME LIMIT =====================
export const checkResumeLimit = async (req, res) => {
    try {
        const userId = req.userId;
        ...
        const freeTemplates = ["digital-pro", "modern-two-column"];
        if (templateId && freeTemplates.includes(templateId)) {
