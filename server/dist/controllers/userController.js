...
export const getUserById = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId).select('name email mobile role plan planExpiresAt planRenewable unlockedTemplates downloadCount downloadLimit lastDownloadReset personalInfoLocked personalInfoEditCount firstDownloadedResumeId linkedinOptimizationCount linkedinPaid atsScore linkedinScore createdAt updatedAt');
        if (!user) {
...
export const updateProfile = async (req, res) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(userId, { name, email, mobile }, { new: true, runValidators: true });
        if (!updatedUser) {
            return res.status(404).json({ message: "User not found" });
        }
...
export const updateMobile = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(userId, { mobile }, { new: true }).select("-password");
        return res.status(200).json({
...
export const sendPaymentSuccessEmail = async (userEmail, userName, paymentDetails) => {
    try {
        const planBenefits = {
        ...
        const benefits = planBenefits[paymentDetails.plan] || ['Full access to premium features'];
        await resend.emails.send({
            from: "CareerBlueprint <info@careerblueprint.co.in>",
            to: userEmail,
...
export const updateScores = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            ...
            linkedinScore: user.linkedinScore ?? null,
        });
    }
    catch (error) {
