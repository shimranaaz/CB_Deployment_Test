...
export const checkLinkedInAccess = async (req, res) => {
    try {
...
export const useLinkedInOptimization = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
        ...
        await user.save();
        console.log(`✅ LinkedIn optimization used: user=${userId}, count=${user.linkedinOptimizationCount}/${MAX_USES}`);
        res.status(200).json({
            success: true,
            usageCount: user.linkedinOptimizationCount,
