...
export const createResumeForUser = async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        const requestingAdmin = await AdminUser.findById(req.userId);
...
CRITICAL: Return ONLY the JSON object. No markdown. No backticks. No explanations.`;
                    ...
                    parsedResumeData = {
                        ...
                        skills: Array.isArray(rawParsed.skills)
                            ? rawParsed.skills.filter((s) => typeof s === 'string' && s.trim())
                            : [],
                        personal_info: {
                            image: '',
...
export const updateResumeForUser = async (req, res) => {
    try {
        ...
        const resume = await Resume.findOneAndUpdate({ _id: resumeId, userId }, {
            $set: {
                ...resumeData,
                ...(title && { title }),
                ...(template && { template }),
...
export const transferResume = async (req, res) => {
    try {
        const { resumeId } = req.params;
        ...
        const resume = await Resume.findByIdAndUpdate(resumeId, { $set: { userId: newUserId } }, // ← just change the userId
        { new: true });
        if (!resume) {
            res.status(404).json({ message: 'Resume not found' });
            return;
...
export const deleteResumeForUser = async (req, res) => {
    try {
        const { userId, resumeId } = req.params;
...
export const publishResumeForUser = async (req, res) => {
    try {
        const { resumeId } = req.params;
...
export const unpublishResumeForUser = async (req, res) => {
    try {
        const { resumeId } = req.params;
        const resume = await Resume.findById(resumeId);
        if (!resume) {
